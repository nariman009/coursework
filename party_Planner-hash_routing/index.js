...
function renderEvents() {
    ...
    document.getElementById('events-container').addEventListener('click', function(event) {
        if (event.target.classList.contains('event-name')) {
            ...
            if (location.hash === `#event-${eventId}`) {
                ...
                location.hash = `#event-${eventId}`; // Update the hash
            }
        }
    });
    ...
    function handleHashChange() {
        ...
        // If hash matches an event, show its description
...
// Call this function at the end of your script or after fetching the events
checkInitialHash();